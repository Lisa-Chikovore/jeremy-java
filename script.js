...
Again, you start off with 6 eggs, 3 litres of milk, and 4 kilos of flour. It takes 6 eggs, 2 litres of milk, and 4 kilos of flour to make 2 cakes. Since you have 1 litre of milk left, you can use your R40 rand to buy 3 eggs (R9 = 3 × R3) and 2 kilos of flour (R30 = 2 × R15), and still have R1 left over. You can bake another cake with the bought ingredients to bake a total of 3 cakes.`,
    ...
    starterCode: `public class Main {
    public static void main(String[] args) {
        // Read the available ingredients, money and recipe.
        // Calculate and print the maximum number of cakes.
    }
}`,
...
const challengeHints = Array.from(
  ...
  section => Array.from(
    section.querySelectorAll(":scope > p, :scope > ol > li"),
    hint => hint.innerHTML
  )
);
...
function updateDocumentation() {
  ...
  if (
    ...
    documentationContent.insertAdjacentHTML(
      "beforeend",
...
// Compare code tokens, ignoring comments and formatting
// while preserving the contents of string and character literals.
function hintCodeSignature(code) {
  const tokens = code.match(/"""[\s\S]*?"""|"(?:\\[\s\S]|[^"\\])*"|'(?:\\[\s\S]|[^'\\])*'|\/\/[^\n\r]*|\/\*[\s\S]*?(?:\*\/|$)|[A-Za-z_$][\w$]*|\d+(?:\.\d+)?|[^\s]/g) || [];
...
function revealNextHint() {
  ...
  const signature = hintCodeSignature(codeEditor.value);

  // Empty code, comment-only code, and unchanged starter code
  // do not qualify for a hint.
  if (
    signature === "[]" ||
    signature === hintCodeSignature(challenge.starterCode)
  ) {
    feedbackMessage.textContent =
      "Write your own code before requesting a hint.";

    return;
  }

  // Clicking Run or Submit again with the same code
  // must not reveal another hint.
  if (signature === lastHintCode) {
    feedbackMessage.textContent =
      "Change your code before trying again to unlock another hint.";
  ...
  if (revealedHintCount >= hints.length) {
    feedbackMessage.textContent =
      "All hints have been revealed. Review them and revise your code.";

    return;
  }

  lastHintCode = signature;
  revealedHintCount += 1;

  updateDocumentation();
  saveChallengeState();

  const latestHint = documentationContent.querySelector(
    "ol > li:last-child"
  );

  if (latestHint) {
    latestHint.scrollIntoView({
      block: "nearest",
    });
  }
}
...
function startAdventure() {
  ...
  explorer.style.transform = "translate(-50%, -50%)";
  ...
  delayedActionId = window.setTimeout(() => {
...
function clearDelayedAction() {
  if (delayedActionId !== null) {
    window.clearTimeout(delayedActionId);
...
function moveExplorerToCheckpoint() {
  ...
  requestAnimationFrame(() => {
    explorer.style.left = checkpoint.left;
    explorer.style.top = checkpoint.top;
  });
...
function startCurrentLevel() {
  ...
  openChallenge(currentChallengeIndex);
}
...
function completeCurrentLevel() {
  ...
  progressDisplay.textContent = `${completedLevels}/8`;
  ...
  if (nextChallenge) {
    currentLevel = nextChallenge.level;
    levelDisplay.textContent = String(currentLevel);
  }
...
function clearSavedChallenge() {
  sessionStorage.removeItem(CHALLENGE_STATE_KEY);
}
...
function restoreChallengeState() {
  ...
  try {
    ...
    if (
      !Number.isInteger(state.currentChallengeIndex) ||
    ...
    currentChallengeIndex = state.currentChallengeIndex ?? 0;
    ...
    if (typeof state.editorCode === "string") {
      codeEditor.value = state.editorCode;
    }

    if (typeof state.programInput === "string") programInput.value = state.programInput;

    revealedHintCount = Number.isInteger(state.revealedHintCount)
      ? Math.max(0, state.revealedHintCount)
      : 0;

    // Older saves did not store the code that earned the latest hint.
    lastHintCode = typeof state.lastHintCode === "string"
      ? state.lastHintCode
      : (
          revealedHintCount > 0
            ? hintCodeSignature(codeEditor.value)
            : null
        );
    ...
    console.error("Could not restore challenge:", error);
...
function openChallenge(index) {
  executionGeneration += 1;
  programInput.value = runnerConfig?.samples[challenges[index]?.id] || "";
  revealedHintCount = 0;
  lastHintCode = null;
  ...
  challengePoints.textContent = String(challenge.points);
...
async function executeEditor(submit) {
  const challenge = challenges[currentChallengeIndex];
  if (!challenge || gameFinished || executionPending || codeEditor.disabled) return;
  if (!codeEditor.value.trim()) {
    consoleOutput.textContent = "Write a Java program first.";
    feedbackMessage.textContent = "";
    return;
  }
  if (!runnerConfig) {
    consoleOutput.textContent = "Start the game with start-game.cmd, then open http://127.0.0.1:3210. The Java runner is not connected.";
    return;
  }
  saveChallengeState();
  const generation = executionGeneration;
  executionPending = true;
  codeEditor.disabled = runButton.disabled = submitButton.disabled = true;
  programInput.disabled = true;
  consoleOutput.textContent = submit ? "Checking your solution..." : "Running...";
  feedbackMessage.textContent = "";
  try {
    const response = await fetch("/api/run", {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-Runner-Token": runnerConfig.token },
      body: JSON.stringify({ code: codeEditor.value, input: programInput.value,
        challengeId: submit ? challenge.id : null })
    });
    const result = await response.json();
    if (generation !== executionGeneration) return;
    if (!response.ok || result.error) throw new Error(result.error || "The Java runner could not finish.");
    resetChallengeControls();
    const output = result.stdout || "";
    const errors = result.stderr || "";
    consoleOutput.textContent = output + (errors ? (output && !output.endsWith("\n") ? "\n" : "") + errors : "");
    if (!output && !errors) consoleOutput.textContent = "Program finished without printing anything.";
    consoleOutput.scrollIntoView({ block: "nearest" });
    if (submit && result.passed) {
      consoleOutput.textContent = `Passed ${result.testsPassed}/${result.totalTests} tests.\n\nLast test output:\n${output}`;
      awardChallenge(challenge);
    } else if (result.status === "service_error") {
      feedbackMessage.textContent = "The Java runner needs attention. Your hints have not changed.";
    } else if (result.status !== "ok") {
      if (result.failedTest) {
        consoleOutput.textContent += `\n\nTest ${result.failedTest.number} failed.\nInput:\n${result.failedTest.input}\nExpected output${challenge.id === 16 ? " (any valid optimal path is accepted)" : ""}:\n${result.failedTest.expected}`;
      }
      feedbackMessage.textContent = "Review the console and edit your code before trying again.";
      revealNextHint();
    } else {
      feedbackMessage.textContent = "Program ran successfully. Submit Solution checks whether it solves the challenge.";
    }
  } catch (error) {
    if (generation === executionGeneration) {
      consoleOutput.textContent = `Runner error: ${error.message}`;
      feedbackMessage.textContent = "Make sure start-game.cmd is running, then refresh the page. Your hints have not changed.";
    }
  } finally {
    executionPending = false;
    // A successful submission advances the index and stays locked during transition.
    if (generation === executionGeneration && currentChallengeIndex === challenges.indexOf(challenge) && !gameFinished) resetChallengeControls();
  }
}

function runCode() {
  return executeEditor(false);
}

function submitSolution() {
  return executeEditor(true);
}
...
function awardChallenge(challenge) {
  ...
  const completedLevel = challenge.level;
  ...
  delayedActionId = window.setTimeout(() => {
    if (
      nextChallenge &&
      nextChallenge.level === completedLevel
    ) {
      openChallenge(currentChallengeIndex);
      return;
    }
...
function finishAdventure() {
  ...
  challengeLevel.textContent = "Adventure Complete";
  ...
  challengePoints.textContent = String(score);

  codeEditor.value = `// Final score: ${score}
// Levels completed: 8
...
// Explorer Certification achieved`;
  ...
  consoleOutput.textContent = `Final score: ${score}`;

  feedbackMessage.textContent =
    "Excellent work, Explorer! You completed the entire Java adventure.";

  progressDisplay.textContent = "8/8";
}
...
function handleEditorTab(event) {
  ...
  codeEditor.selectionStart = start + indentation.length;
  codeEditor.selectionEnd = start + indentation.length;
}
...
async function initializeGame() {
  ...
  programInput.addEventListener("input", saveChallengeState);
  try {
    const response = await fetch("/api/config");
    if (response.ok) runnerConfig = await response.json();
  } catch (_) { /* Run explains how to start the local runner. */ }
...
function verifyRequiredElements() {
  ...
  if (
    ...
    console.error("The challenges array is missing.");
    return false;
  }

  if (typeof javaDocumentation === "undefined") {
    console.error("The javaDocumentation object is missing.");
    return false;
  }
